...
public class FileUtils {
	...
	/*
	 * String uploadFolder : 서버측의 업로드 된 폴더 C:\\dev\\upload\\product\\
...
//		File클래스  : 파일과 폴더관련 작업하는 기능
		...
		//3f281a3b-7d7d-4e5d-9fd0-37469b8bc32f_jordan1bw.jpg
		...
		try {
			...
			//Thumbnail 작업
			//원본이미지가 파일크기가 커서, 브라우저에 리스트롤 사용시 로딩되는 시간이 길어진다ㅣ
			...
			if(checkImageType(saveFile)) { //첨부된 파일이 이미지 일 경우라면
				...
				//원본이미지 파일의 내용을 스트림 방식으로 읽어서 썸네일이미지파일에 쓰는 작업
				//Thumbnailator.createThumbnail(원본이미지 파일의 입력스트림, thumbnail , 100, 100);
	...
	//자바스크립트로 업로드 되는 파일의 확장자를 이용하여 , 이미지파일만 파일첨부 가능하도록 작업을 할수가 있다.
