...
<body class="d-flex flex-column h-100">
								...
								<table class="table table-bordered">
									<tbody>
										...
										<c:forEach items="${list}" var="board">
											<tr>
												<td>${board.bno}</td>
												<td><a class="move" href="#" data-bno="${board.bno}">${board.title}</a></td> <!-- 클래스명 move는 제목과 관련 -->
												<td> ${board.writer}</td>
												<td>
										...
										<nav aria-label="...">
											<ul class="pagination">
												...
												<c:if test="${pageMaker.next}">
													<li class="page-item">
														<a href="/board/list?pageNum=${pageMaker.endPage + 1}" class="page-link">Next</a>
													</li>
												</c:if>
											</ul>
										</nav>
	...
	<script>
		...
		// <a class="move" href="#" data-bno="게시물 번호"></a>
		...
		Array.from(moves).forEach(function(move) {
			...
			move.addEventListener("click", function(event) {
				...
				//bno 제거작업
				//목록에서 제목 클릭후 게시물읽기에서 뒤로버튼에 의하여 목록으로 돌아옴
				...
				let bno = event.target.dataset.bno // data-bno -> dataset.bno 
        ...
        newInput.setAttribute("value", bno); // bno는 변수명
				actionForm.appendChild(newInput);
				
				actionForm.setAttribute("action", "/board/get"); // /board/list -> /board/get 전송
				actionForm.submit(); 
			});
		});
	</script>
